...
export default class ReactGrid extends React.Component {
    ...
    constructor(props){
        ...
        this.state = {
          ...
          data: JSON.parse(this.makeAPIcall("www.boswemianrhapsody.me/api/sort/artists/1/artist/asc"))
        };
        
        this.updateGridData = this.updateGridData.bind(this);
        this.makeSortFiler = this.makeSortFilter.bind(this);
...
//       alert("Sort: " + sort + " filter " + filter + " page "  + current);
       this.setState({data: this.makeAPIcall("www.boswemianrhapsody.me/api/sort/" + type + "/" + page + "/" + filter + "/" + sort)});
        // console.log(props);
   }
...
//        var artist = this.makeAPIcall("boswemianrhapsody.me/api/album/" + data.artist_album_pairs[0].aa_id);
        ...
        return (
            <div key={data.id} className="col-sm-4 col-xs-12 sweGridItem">
                ...
                <p className="sweGridItemContent">Artists: <a onclick={openArtistModal(artist.id)}>{artist.name}</a></p>
