...
export default class SWESearch extends React.Component {
  ...
  pageChange(page) {
	...
	getHits(target, targetLength, raw, attr) {
		...
		console.log("Raw: " + raw);
		if (raw != null && ((index = raw.toLowerCase().indexOf(target)) != -1)) {
			match = raw.substring(index, index + targetLength);
			result = (<div key={match+attr+raw.length} className="col-xs-6" style={style.searchHit}>{attr}: contains <span style={style.searchHighlight}>{match}</span><br/></div>);
		}
